...
struct Current: View {
    ...
    @State private var animator = BoatAnimator()
    ...
    var body: some View {
        canvas
            ...
            .frame(height: 56)
            // Haptic only on a real forward step; the glide itself is driven off
            // the TimelineView clock in `draw` (see BoatAnimator).
    ...
    @ViewBuilder private var canvas: some View {
        if Motion.reduceMotion {
            Canvas { ctx, size in draw(&ctx, size, time: 0, phase: 0, bob: 0, animated: false) }
        } else {
            TimelineView(.animation) { timeline in
                let t = timeline.date.timeIntervalSinceReferenceDate
                let phase = 2 * .pi * (t.truncatingRemainder(dividingBy: 12) / 12)
                let bob = 1.4 * sin(2 * .pi * t / 2.6)
                Canvas { ctx, size in draw(&ctx, size, time: t, phase: phase, bob: bob, animated: true) }
            }
        }
    }

    private func draw(_ ctx: inout GraphicsContext, _ size: CGSize, time: Double, phase: CGFloat, bob: CGFloat, animated: Bool) {
        let w = size.width - inset * 2
        guard w > 0 else { return }
        let midY = size.height / 2
        let progress = animated
            ? animator.progress(at: time, from: fraction(max(0, currentStage - 1)),
                                to: fraction(currentStage), key: currentStage, duration: Motion.river)
            : fraction(currentStage)
        ...
        if progress > 0.001 {
            let travelled = full.trimmedPath(from: 0, to: progress)
...
/// Time-driven glide for the raft. Kept in a reference box (like the river
/// caches): a `withAnimation`'d `@State` read only inside a `Canvas` doesn't
/// interpolate frame to frame, so progress is computed from the TimelineView
/// clock instead. On first appearance it glides in from the previous node.
private final class BoatAnimator {
    ...
    func progress(at t: Double, from newFrom: CGFloat, to newTo: CGFloat, key newKey: Int, duration newDuration: Double) -> CGFloat {
        if newKey != key {
            from = key == Int.min ? newFrom : value(at: t) // continue from where it is on a mid-glide change
            to = newTo
            start = t
    ...
    private func value(at t: Double) -> CGFloat {
        let u = duration <= 0 ? 1 : max(0, min(1, CGFloat((t - start) / duration)))
        let inv = 1 - u
        return from + (to - from) * (1 - inv * inv * inv) // easeOut cubic
    }
}
